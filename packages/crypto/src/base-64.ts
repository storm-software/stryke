...
// ported from https://github.com/hi-ogawa/js-utils
 ...
 */
export function toBase64(input: Uint8Array): Uint8Array {
  ...
  for (let i = 0; i < chunkLen; i++) {
    ...
    result[4 * i + 0] = ENC[(c >> 18) & 0b111111]!;
    result[4 * i + 1] = ENC[(c >> 12) & 0b111111]!;
    result[4 * i + 2] = ENC[(c >> 6) & 0b111111]!;
    result[4 * i + 3] = ENC[(c >> 0) & 0b111111]!;
  }
  ...
  switch (xLen % 3) {
    case 1: {
      const c = input[3 * i + 0]! << 16;
      result[4 * i + 0] = ENC[(c >> 18) & 0b111111]!;
      result[4 * i + 1] = ENC[(c >> 12) & 0b111111]!;
      result[4 * i + 2] = ENC_PAD;
      result[4 * i + 3] = ENC_PAD;
      break;
    }
    case 2: {
      const c = (input[3 * i + 0]! << 16) | (input[3 * i + 1]! << 8);
      result[4 * i + 0] = ENC[(c >> 18) & 0b111111]!;
      result[4 * i + 1] = ENC[(c >> 12) & 0b111111]!;
      result[4 * i + 2] = ENC[(c >> 6) & 0b111111]!;
      result[4 * i + 3] = ENC_PAD;
 ...
 */
export function fromBase64(input: Uint8Array): Uint8Array {
  ...
  while (
    padLen < 2 &&
    padLen < input.length &&
    input[input.length - 1 - padLen] === ENC_PAD
  ) {
    padLen++;
  }
  ...
  for (let i = 0; i < chunkLen; i++) {
    const c =
      ...
      (DEC[input[4 * i + 2]!]! << 6) |
      (DEC[input[4 * i + 3]!]! << 0);
    result[3 * i] = (c >> 16) & 0xff;
    result[3 * i + 1] = (c >> 8) & 0xff;
    result[3 * i + 2] = (c >> 0) & 0xff;
  }
  ...
  switch (xLen % 3) {
    case 1: {
      const c =
        (DEC[input[4 * i + 0]!]! << 18) | (DEC[input[4 * i + 1]!]! << 12);
      result[3 * i] = (c >> 16) & 0xff;
      break;
    }
    case 2: {
